...
const ActivitiesPage = () => {
  ...
  useEffect(() => {
    const fetchActivities = async () => {
      try {
        const response = await axios.get("http://localhost:8000/activities"); // Adjust URL as needed
  ...
  const handleActivityClick = (activityId) => {
    navigate(`/activitybooking/${activityId}`);
  };
      ...
      {activities.length === 0 ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {activities.map((activity) => (
            ...
            >
              <img
                src={`http://localhost:8000/${activity.image}`} // Adjust if using a different static file path
                alt={activity.name}
